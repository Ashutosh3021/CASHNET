...
class StrictComplaintTests(unittest.TestCase):
 def test_exact_schema_and_cross_dataset_accounts(self):
  ...
  self.assertEqual(strict.validate(rows,meta),[]); self.assertEqual(tuple(rows[0]),strict.TOP); self.assertTrue(registry)
  self.assertTrue(any(t["method"]=="UPI" and "@cashnet.invalid" in t["from_account"] for r in rows for t in r["transaction_details"]["transactions"]))
