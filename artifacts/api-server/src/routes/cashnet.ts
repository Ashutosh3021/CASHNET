...
router.get("/reports/:caseId", (req, res) => { const d = detail(req.params.caseId); if (!d) return res.status(404).json({ error: "Case not found", caseId: req.params.caseId }); const historical = detectHotspots(syntheticGeoData.records, syntheticGeoData.atms, syntheticGeoData.branches); const historicalSummary = { transactions: syntheticGeoData.records.length, hotspots: historical.length, topHotspot: [...historical].sort((a, b) => b.historicalScore - a.historicalScore)[0]?.clusterId ?? "NONE", dataSource: "SYNTHETIC" }; res.json({ case: d, sections: [ { title: "CASE SUMMARY", status: "INCLUDED", provenance: { sourceType: "USER_PROVIDED_DATA", description: "Case intake from user report" } }, { title: "COMPLAINT", status: "INCLUDED", provenance: { sourceType: "USER_PROVIDED_DATA", description: "User-provided complaint narrative" } }, { title: "ACCOUNT ANALYSIS", status: "INCLUDED", provenance: { sourceType: "SYNTHETIC", description: "Synthetic bank account data for demonstration" } }, { title: "TRANSACTION HISTORY", status: "INCLUDED", provenance: { sourceType: "SYNTHETIC", description: "Synthetic transaction data for demonstration" } }, { title: "FUND FLOW", status: "INCLUDED", provenance: { sourceType: "SYNTHETIC", description: "Synthetic fund flow graph for demonstration" } }, { title: "FIAT TO CRYPTO CONVERSION TIMESTAMP", status: "INCLUDED", provenance: { sourceType: "SYNTHETIC", description: "Synthetic conversion timestamp" } }, { title: "CRYPTO ANALYSIS", status: "INCLUDED", provenance: { sourceType: "MODEL_INFERENCE", description: "Model 182 output - Crypto/VASP classification", modelVersion: "182" } }, { title: "VASP ATTRIBUTION", status: "INCLUDED", provenance: { sourceType: "PUBLIC_DATA", description: "Public blockchain explorer labels", confidence: 0.9 } }, { title: "RISK ANALYSIS", status: "INCLUDED", provenance: { sourceType: "MODEL_INFERENCE", description: "Model 184 risk scoring output", modelVersion: "184" } }, { title: "PREDICTIVE HOTSPOTS", status: "INCLUDED", provenance: { sourceType: "MODEL_INFERENCE", description: "Geospatial hotspot detection using DBSCAN clustering", modelVersion: "geospatial-v1" } }, { title: "ACTIONABLE INTELLIGENCE", status: "INCLUDED", provenance: { sourceType: "MODEL_INFERENCE", description: "Aggregated recommendations from multiple models" } }, { title: "INTERVENTION REQUEST", status: "INCLUDED", provenance: { sourceType: "USER_PROVIDED_DATA", description: "Investigator-initiated intervention request" } }, { title: "AUDIT LOG", status: "INCLUDED", provenance: { sourceType: "USER_PROVIDED_DATA", description: "System audit trail" } }, { title: "HISTORICAL SUSPICIOUS ACTIVITY", status: "INCLUDED", provenance: { sourceType: "SYNTHETIC", description: "Synthetic demonstration data - not real transactions" }, summary: historicalSummary }], disclaimer: "Analytical prediction requiring investigator validation. Some sections use synthetic demonstration data. Model outputs are MODEL_INFERENCE and require human review. NCRP and SAHYOG are NOT connected.", provenance: { generatedAt: new Date().toISOString(), dataSource: "MIXED", containsSyntheticData: true, containsModelInference: true, ncrpConnected: false, sahyogConnected: false } }); });
