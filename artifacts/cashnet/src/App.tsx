...
function Sidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return <aside className={`${collapsed ? 'w-[72px]' : 'w-[238px]'} fixed inset-y-0 left-0 z-30 hidden shrink-0 flex-col bg-[hsl(var(--sidebar))] text-[hsl(var(--sidebar-foreground))] transition-all duration-200 lg:flex`}>
    <div className="flex h-[68px] items-center border-b border-[hsl(var(--sidebar-border))] px-4">
      <Link href="/dashboard" className="flex min-w-0 items-center gap-3" data-testid="link-brand">
...
function Topbar({ onMenu }: { onMenu: () => void }) {
  ...
  const section = location === '/dashboard' ? 'Overview' : titleCase(location.split('/')[1]);
...
function DashboardPage() {
  ...
  const cityComplaints = [
    { city: 'Bengaluru', complaints: 353 },
    { city: 'Delhi NCR', complaints: 108 },
    { city: 'Mumbai', complaints: 96 },
    { city: 'Hyderabad', complaints: 87 },
  ];
  const maxComplaints = cityComplaints[0].complaints;
  return <div className="enter"><PageHead kicker="Signal desk / live" title="Overview" description="A compact operating picture of active fraud, conversion windows and intervention-ready intelligence." action={<Link href="/cases" className="flex items-center justify-center gap-2 bg-slate-800 px-4 py-2.5 text-xs font-bold text-amber-300 hover:bg-slate-700" data-testid="link-open-cases"><Fingerprint size={15} /> Open case desk</Link>} />
    <div className="mb-5 grid gap-3 lg:grid-cols-4"><Metric label="Active cases" value={metric('activeCases')} detail="Across current queue" tone="slate" icon={Fingerprint} /><Metric label="Exposure tracked" value={metric('totalExposure') === '—' ? compactMoney(caseList.reduce((a, c) => a + c.amount, 0)) : metric('totalExposure')} detail="Fiat equivalent" tone="amber" icon={BarChart3} /><div className="col-span-2 border border-slate-200 bg-white p-4" data-testid="card-city-complaints"><div className="flex items-start justify-between"><div><div className="text-[10px] font-bold uppercase tracking-[.1em] text-slate-400">City complaint volume</div><div className="mt-1 text-[10px] text-slate-500">Reported financial fraud complaints</div></div><BarChart3 size={15} className="text-cyan-700" /></div><div className="mt-4 space-y-2.5">{cityComplaints.map((item, index) => <div className="grid grid-cols-[76px_1fr_32px] items-center gap-2" key={item.city}><span className="truncate text-[10px] font-semibold text-slate-600">{item.city}</span><div className="h-2.5 overflow-hidden bg-slate-100"><div className={`h-full ${index === 0 ? 'bg-cyan-600' : 'bg-cyan-400/80'}`} style={{ width: `${item.complaints / maxComplaints * 100}%` }} /></div><span className="font-mono-data text-right text-[10px] font-bold text-slate-700">{item.complaints}</span></div>)}</div></div></div>
...
function CasesPage() {
  ...
  const [showForm, setShowForm] = useState(false); const [query, setQuery] = useState(''); const [form, setForm] = useState({ title: '', fraudType: 'UPI scam', amount: '', description: '', victimCity: '', victimState: '', victimLat: '', victimLng: '', pinCode: '', account: '', wallet: '' });
  ...
  const captureLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => { setForm(f => ({ ...f, victimLat: String(pos.coords.latitude), victimLng: String(pos.coords.longitude) })); setGeoLoading(false); },
  ...
  const submit = (event: React.FormEvent) => { event.preventDefault(); createCase.mutate({ data: { ...form, amount: Number(form.amount), victimLat: form.victimLat ? Number(form.victimLat) : undefined, victimLng: form.victimLng ? Number(form.victimLng) : undefined } }, { onSuccess: () => { setShowForm(false); setForm({ title: '', fraudType: 'UPI scam', amount: '', description: '', victimCity: '', victimState: '', victimLat: '', victimLng: '', pinCode: '', account: '', wallet: '' }); queryClient.invalidateQueries({ queryKey: cases.queryKey }); } }); };
...
function CaseWorkspacePage() {
  ...
  return <div className="enter"><div className="mb-5 flex flex-wrap items-center gap-2 text-[10px] font-mono-data uppercase tracking-[.12em] text-slate-500"><Link href="/cases" className="text-cyan-700 hover:underline" data-testid="link-back-cases">Case desk</Link><ChevronRight size={12} /><span className="text-slate-800">{item.reference}</span><Pill tone="amber">{item.state || item.status}</Pill></div><PageHead kicker={`Unified case workspace / ${item.reference}`} title={item.title} description={`${titleCase(item.fraudType)} · ${item.city || 'Location unknown'} · opened ${dateOnly(item.updatedAt || item.conversionAt)}`} action={<div className="flex flex-wrap gap-2"><button onClick={runAnalysis} disabled={analyze.isPending || analysisLoading} className="flex items-center gap-2 border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:border-cyan-400 disabled:opacity-50" data-testid="button-run-analysis">{analyze.isPending || analysisLoading ? <Activity size={14} className="animate-pulse" /> : <Zap size={14} className="text-amber-500" />}{analyze.isPending || analysisLoading ? 'Running pipeline' : 'Run analysis'}</button><button onClick={() => setComplaintOpen(!complaintOpen)} className="flex items-center gap-2 bg-amber-400 px-3 py-2 text-xs font-bold text-slate-900" data-testid="button-add-complaint"><Upload size={14} /> Add report</button></div>} />
    ...
    {!analysisReady && <Panel title={analysisLoading ? 'Analysis in progress' : 'Analysis required'} eyebrow="Investigation report"><div className="flex min-h-[240px] flex-col items-center justify-center p-7 text-center"><div className="mb-3 flex size-12 items-center justify-center rounded-full bg-cyan-100 text-cyan-700">{analysisLoading ? <Activity size={20} className="animate-pulse" /> : <Zap size={20} />}</div><h2 className="text-base font-bold text-slate-800">{analysisLoading ? 'Generating the investigation report' : 'Run analysis to generate the report'}</h2><p className="mt-2 max-w-md text-xs leading-5 text-slate-500">{analysisLoading ? 'Account, wallet, risk, complaint and transaction intelligence will appear after the analysis completes.' : 'The investigation chain and all linked intelligence remain hidden until you run analysis for this case.'}</p></div></Panel>}
    <div className={analysisReady ? '' : 'hidden'}>
...
function FundFlowPage() {
  ...
  const data = flow.data as FundFlow;
  const nodes = data.nodes || [], edges = data.edges || [], timeline = data.timeline || [];
  const kindForFilter = (kind: string) => ({ MULE_ACCOUNT: 'ACCOUNTS', BANK_ACCOUNT: 'ACCOUNTS', CRYPTO_WALLET: 'CRYPTO', VASP: 'VASP', FOREIGN_ENTITY: 'FOREIGN', CASH_OUT_LOCATION: 'CASH-OUT' } as Record<string, string>)[kind] || 'ALL';
  const visibleNodes = nodes.filter((n) => (filter === 'ALL' || kindForFilter(n.kind) === filter) && (!suspiciousOnly || n.risk >= 70));
  const visibleIds = new Set(visibleNodes.map((n) => n.id));
  const visibleEdges = edges.filter((e) => visibleIds.has(e.source) && visibleIds.has(e.target) && (!suspiciousOnly || e.risk >= 70));
  const selectedTransaction = selectedEdge ? edges.find((e) => e.id === selectedEdge) : null;
  const selectedEntity = selectedNode ? nodes.find((n) => n.id === selectedNode) : null;
  const nodeName = (id: string) => nodes.find((n) => n.id === id)?.label || id;
  const riskLabel = (risk: number) => risk >= 90 ? 'CRITICAL' : risk >= 70 ? 'HIGH' : risk >= 40 ? 'MEDIUM' : 'LOW';
  const edgeForEvent = (event: any) => edges[Math.max(0, timeline.findIndex((item) => item.id === event.id))];
  const selectEvent = (event: any) => { const edge = edgeForEvent(event); if (edge) { setSelectedEdge(edge.id); setSelectedNode(null); } };
  return <div className="enter"><PageHead kicker={`Graph / ${caseId}`} title="Fund flow" description="Unified graph and synchronized event timeline. Select a node, transfer or timestamp to inspect the evidence chain." action={<Pill tone="cyan"><Activity size={12} /> Live graph feed</Pill>} /><div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4"><Metric label="Nodes" value={String(nodes.length)} detail="Known entities" icon={Network} tone="slate" /><Metric label="Edges" value={String(edges.length)} detail="Observed transfers" icon={ArrowUpRight} tone="cyan" /><Metric label="Graph risk" value={String((data.metrics as any)?.risk ?? '—')} detail="Provider metric" icon={Radar} tone="red" /><Metric label="Tracked value" value={compactMoney(edges.reduce((a, e) => a + e.amount, 0))} detail="Across graph edges" icon={BarChart3} tone="amber" /></div><div className="mb-5 grid gap-5 xl:grid-cols-[1.2fr_.8fr]"><Panel title="Investigation insight" eyebrow="CASE SIGNALS"><div className="grid gap-3 sm:grid-cols-2"><div className="border-l-2 border-red-400 pl-3 text-sm font-bold text-slate-800">High-risk fund movement detected.</div><div className="text-xs leading-5 text-slate-500">• Rapid movement between linked accounts<br />• Crypto conversion detected<br />• Multiple connected entities<br />• Potential cash-out endpoint identified</div></div></Panel><Panel title="Data source" eyebrow="PROVENANCE"><div className="flex items-start gap-2 text-xs text-slate-600"><CheckCircle2 size={15} className="mt-0.5 text-emerald-600" /><div><div className="font-bold text-slate-800">CASHNET Case Dataset</div><div>Verified against project source data</div><div className="mt-1 font-mono-data text-[10px] text-slate-400">✓ {edges.length}/{edges.length} transactions · ✓ {nodes.length}/{nodes.length} entities · Current session</div></div></div></Panel></div><div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]"><Panel title="Entity graph" eyebrow="Synchronized / select a node or transfer" className="scanline" action={<div className="flex items-center gap-1"><button className="border border-slate-300 p-1.5" title="Zoom in" onClick={() => setZoom((z) => Math.min(1.5, z + .1))}><ZoomIn size={13} /></button><button className="border border-slate-300 p-1.5" title="Zoom out" onClick={() => setZoom((z) => Math.max(.7, z - .1))}><ZoomOut size={13} /></button><button className="border border-slate-300 p-1.5" title="Reset view" onClick={() => setZoom(1)}><Maximize2 size={13} /></button></div>}><div className="mb-3 flex flex-wrap gap-1.5">{['ALL','ACCOUNTS','CRYPTO','VASP','FOREIGN','CASH-OUT'].map((item) => <button key={item} onClick={() => setFilter(item)} className={`border px-2 py-1 text-[10px] font-bold ${filter === item ? 'border-cyan-600 bg-cyan-50 text-cyan-800' : 'border-slate-200 text-slate-500'}`}>{item}</button>)}<label className="ml-auto flex items-center gap-1 text-[10px] text-slate-500"><input type="checkbox" checked={suspiciousOnly} onChange={(e) => setSuspiciousOnly(e.target.checked)} /> Suspicious only</label></div><div className="relative min-h-[430px] overflow-hidden bg-[hsl(var(--sidebar))] p-5" data-testid="fund-flow-graph"><div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(circle at 1px 1px, #84a3b8 1px, transparent 0)', backgroundSize: '22px 22px' }} /><svg className="pointer-events-auto absolute inset-0 size-full" style={{ transform: `scale(${zoom})`, transformOrigin: 'center' }}>{visibleEdges.map((edge) => { const source = nodes.find((node) => node.id === edge.source); const target = nodes.find((node) => node.id === edge.target); if (!source || !target) return null; const active = selectedEdge === edge.id; return <g key={edge.id} onClick={() => { setSelectedEdge(edge.id); setSelectedNode(null); }} className="cursor-pointer"><line x1={`${source.x}%`} y1={`${source.y}%`} x2={`${target.x}%`} y2={`${target.y}%`} stroke={active ? '#f6c453' : edge.risk >= 70 ? '#e87575' : edge.conversion ? '#f6c453' : '#4f8190'} strokeWidth={active ? 4 : edge.risk >= 70 ? 2 : 1} strokeDasharray={edge.conversion ? '5 4' : undefined} /><text x={`${(source.x + target.x) / 2}%`} y={`${(source.y + target.y) / 2}%`} fill={active ? '#fff1b8' : '#b7c9d2'} fontSize="9" textAnchor="middle">{compactMoney(edge.amount)}</text></g>; })}</svg>{visibleNodes.map((node) => <button key={node.id} onClick={() => { setSelectedNode(node.id); setSelectedEdge(null); }} className="absolute -translate-x-1/2 -translate-y-1/2 text-left" style={{ left: `${node.x}%`, top: `${node.y}%`, transform: `translate(-50%, -50%) scale(${zoom})` }} data-testid={`button-graph-node-${node.id}`}><div className={`flex size-14 items-center justify-center rounded-full border-2 ${node.risk >= 90 ? 'border-red-400 bg-red-500/25' : node.risk >= 70 ? 'border-orange-300 bg-orange-400/20' : node.risk >= 40 ? 'border-amber-300 bg-amber-400/15' : 'border-cyan-300 bg-cyan-400/15'} text-center text-[9px] font-bold text-slate-100 shadow-lg`}><span>{riskLabel(node.risk)}</span></div><div className="mt-1 max-w-24 truncate text-center font-mono-data text-[9px] text-cyan-200">{node.label}</div></button>)}</div>{(selectedEntity || selectedTransaction) && <div className="mt-3 border border-cyan-200 bg-cyan-50/60 p-3 text-xs text-slate-700">{selectedEntity ? <><div className="flex items-center justify-between"><b>Entity details</b><button onClick={() => setSelectedNode(null)}><X size={13} /></button></div><div className="mt-2 font-bold text-slate-900">{selectedEntity.label}</div><div className="mt-1 grid grid-cols-2 gap-1 text-[11px]"><span>Type: {titleCase(selectedEntity.kind)}</span><span>Risk: {riskLabel(selectedEntity.risk)} ({selectedEntity.risk})</span><span>Linked entities: {edges.filter((e) => e.source === selectedEntity.id || e.target === selectedEntity.id).length}</span><span>Transactions: {edges.filter((e) => e.source === selectedEntity.id || e.target === selectedEntity.id).length}</span></div><div className="mt-2 text-[11px] text-slate-500">Reason: {selectedEntity.risk >= 70 ? 'Connected to rapid or cross-border movement.' : 'Low-risk origin or observed participant.'}</div></> : <><div className="flex items-center justify-between"><b>Transaction details</b><button onClick={() => setSelectedEdge(null)}><X size={13} /></button></div><div className="mt-2 font-bold text-slate-900">{nodeName(selectedTransaction!.source)} → {nodeName(selectedTransaction!.target)}</div><div className="mt-1 grid grid-cols-2 gap-1 text-[11px]"><span>Amount: {money(selectedTransaction!.amount)}</span><span>Type: {selectedTransaction!.label}</span><span>Timestamp: {dateTime(selectedTransaction!.timestamp)}</span><span>Risk: {riskLabel(selectedTransaction!.risk)}</span></div><div className="mt-2 text-[11px] text-emerald-700">✓ Matches source data · CASHNET Case Dataset</div></>}</div>}</Panel><Panel title="Synchronized timeline" eyebrow={`${timeline.length} events / chronological`}><div className="max-h-[520px] overflow-y-auto divide-y divide-slate-100">{timeline.map((event) => { const edge = edgeForEvent(event); const active = edge && selectedEdge === edge.id; return <button className={`flex w-full gap-3 p-4 text-left hover:bg-cyan-50/30 ${active ? 'bg-cyan-50 ring-1 ring-inset ring-cyan-300' : ''}`} onClick={() => selectEvent(event)} key={event.id} data-testid={`timeline-event-${event.id}`}><div className="relative flex w-16 shrink-0 flex-col items-end"><span className="font-mono-data text-[10px] text-cyan-700">{dateTime(event.time)}</span><span className="mt-1 text-[9px] text-slate-400">{event.category}</span></div><div className="relative border-l border-cyan-300 pl-4"><span className={`absolute -left-[5px] top-1 size-2 rounded-full border border-white ${active ? 'bg-amber-400' : 'bg-cyan-500'}`} /><div className="text-xs font-bold text-slate-800">{event.title}</div><div className="mt-1 text-[11px] leading-5 text-slate-500">{event.detail}</div><div className="mt-2 font-mono-data text-[10px] font-medium text-slate-700">{money(event.amount)}</div></div></button>; })}</div>{!timeline.length && <EmptyState title="Timeline empty" description="No event chronology was returned for this case." />}</Panel></div></div>;
}
