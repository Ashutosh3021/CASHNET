...
class TestProvenance:
    def test_data_source_types(self):
        valid_types = {"PUBLIC_DATA", "AUTHORIZED_API", "USER_PROVIDED_DATA", "SYNTHETIC", "MODEL_INFERENCE"}
    ...
    def test_provenance_fields(self):
        ...
        assert provenance["dataSource"] in {"PUBLIC_DATA", "AUTHORIZED_API", "USER_PROVIDED_DATA", "SYNTHETIC", "MODEL_INFERENCE"}
...
class TestBlockchainProviders:
    def test_provider_status_structure(self):
        ...
        assert status["status"] in {"AVAILABLE", "UNAVAILABLE", "NOT_CONFIGURED", "RATE_LIMITED"}
...
class TestDataIngestion:
    ...
    def test_schema_field_mapping(self):
        ...
        schema_fields = [
            {"name": "latitude", "type": "coordinates", "sourceFields": ["latitude", "lat"]},
        ]
        raw = {"lat": 20.2961}
        # Should map "lat" -> "latitude"
        mapped = None
        for field in schema_fields:
            if raw.get("lat") is not None and "lat" in (field.get("sourceFields") or []):
                mapped = field["name"]
